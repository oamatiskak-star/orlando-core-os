...
- **NA F0 (go per fase):** F1 watchdog-subagent (services/hermes deployen), F3 classifier+governance DRY-RUN, F4 live auto-antwoord (alleen ná dry-run-bewijs, harde default-deny op deploy/merge/migratie/Stripe/prijzen/delete).

**✅ F2 GEBOUWD — Perplexity-vangnet in AI Router:** als de hele provider-keten faalt op capaciteit/limiet (429/timeout/5xx), neemt Perplexity (Sonar) de taak over.
- `ai-os/router/src/providers/perplexity.ts` (OpenAI-compatibel, baseURL api.perplexity.ai, model `sonar`); geregistreerd in `router.ts` providers-map.
- `router.ts`: `sawCapacityIssue`-tracking + last-resort Perplexity-aanroep vóór de "exhausted"-throw; logt `perplexity_fallback_used`; cache + usage geregistreerd.
- `config.ts`: `providers.perplexity` (PERPLEXITY_API_KEY / PERPLEXITY_BASE_URL / PERPLEXITY_FALLBACK_MODEL) + `routing.perplexityFallback` (env `AI_PERPLEXITY_FALLBACK`, default aan).
- Typecheck: geen fouten in router/config/types; perplexity.ts identiek patroon aan werkend openrouter.ts (compileert in Docker).
- **OPEN (Orlando):** `PERPLEXITY_API_KEY` in ai-router env (docker-compose.cli-r.yml) + `ai-router` rebuilden/herstarten op CLI-R.
