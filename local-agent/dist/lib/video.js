...
function run(cmd, args) {
    return new Promise((resolve, reject) => {
        (0, child_process_1.execFile)(cmd, args, { timeout: 300_000 }, (err, _out, stderr) => {
            if (err)
                reject(new Error(`${cmd} error: ${stderr?.slice(0, 500) || err.message}`));
            else
                resolve();
        });
    });
...
function makeTitleImage(imagePath, title, bgColor, width, height) {
...
`;
    return new Promise((resolve, reject) => {
        (0, child_process_1.execFile)('python3', ['-c', pyScript], { timeout: 30_000 }, (err, _out, stderr) => {
            if (err)
                reject(new Error(`PIL error: ${stderr?.slice(0, 300) || err.message}`));
            else
                resolve();
        });
    });
}
async function buildVideo(opts) {
    const { audioPath, outputPath, title, bgColor, isShort } = opts;
    const width = isShort ? 1080 : 1920;
    const height = isShort ? 1920 : 1080;
    const tmpDir = path_1.default.dirname(outputPath);
    const imgPath = path_1.default.join(tmpDir, 'bg.png');
