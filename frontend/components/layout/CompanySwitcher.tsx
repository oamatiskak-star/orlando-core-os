    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute top-full left-0 right-0 mt-1 bg-[#1a1a2e] border border-white/10 rounded-lg z-50 shadow-2xl overflow-hidden max-h-[320px] overflow-y-auto">
            {ROLE_ORDER.map((role) => {
              const items = grouped[role]
              if (!items) return null
              return (
                <div key={role}>
                  <p className="px-3 pt-2.5 pb-1 text-[9px] uppercase tracking-widest text-white/25 font-semibold select-none">
                    {ROLE_LABEL[role]}
                  </p>
                  {items.map((company) => (
                    <button
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-white/30 truncate leading-tight">{company.name}</p>
                      </div>
