  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-white/50 mt-0.5">CFO-rapportages en financiële analyses</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        {REPORTS.map((report) => {
          ...
          return (
            <div key={report.id} className="bg-white/[0.06] border border-white/5 rounded-xl p-5">
              <div className="flex items-start gap-3 mb-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-white/65 mt-1 leading-relaxed">{report.description}</p>
                </div>
              </div>
      ...
      <div className="bg-white/[0.06] border border-white/5 rounded-xl p-5">
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-white/5">
              <th className="pb-3 text-left text-[10px] font-medium text-white/50 uppercase tracking-wider">Periode</th>
              <th className="pb-3 text-right text-[10px] font-medium text-white/50 uppercase tracking-wider">Bedrag</th>
              <th className="pb-3 text-right text-[10px] font-medium text-white/50 uppercase tracking-wider">% van totaal</th>
              <th className="pb-3 text-right text-[10px] font-medium text-white/50 uppercase tracking-wider">Status</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-white/5">
            {AGING_BUCKETS.map((bucket) => (
              <tr key={bucket.label}>
                ...
                <td className="py-3 text-xs text-white/50 text-right">0%</td>
                <td className="py-3 text-right">
                  <span className="bg-white/5 text-white/50 px-2 py-0.5 rounded-full text-[10px] font-medium">
                    Geen data
                  </span>
                </td>
              </tr>
        ...
        <p className="text-[10px] text-white/38 mt-3">
