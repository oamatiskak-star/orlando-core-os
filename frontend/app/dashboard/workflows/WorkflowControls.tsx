...
import { triggerWorkflow, toggleWorkflowStatus, deleteWorkflow, createWorkflow } from './actions'
...
type Workflow = {
  ...
  steps: Array<{ type: string; label: string; [k: string]: string }>
  status: string
...
function WorkflowRow({ wf, defaultSteps }: { wf: Workflow; defaultSteps: Array<{ type: string; label: string }> }) {
  ...
  return (
    <div className="border border-white/5 rounded-xl overflow-hidden">
      ...
      >
        <button className="text-white/45 hover:text-white/50 transition-colors flex-shrink-0" onClick={e => { e.stopPropagation(); setExpanded(!expanded) }}>
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0" onClick={e => e.stopPropagation()}>
          ...
          <button
            onClick={() => { if (confirm(`"${wf.naam}" verwijderen?`)) deleteWorkflow(wf.id) }}
        ...
        <div className="px-4 pb-4 border-t border-white/5 bg-white/[0.01]">
          <div className="pt-3 space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {(wf.steps as Array<{ type: string; label: string }>).map((step, i) => (
                <div key={i} className="flex items-center gap-2 bg-white/5 border border-white/8 rounded-lg px-3 py-1.5">
...
function NieuweWorkflowModal({ onClose }: { onClose: () => void }) {
  ...
  const [steps, setSteps] = useState<Array<{ type: string; label: string }>>([
    { type: 'api_call', label: 'API aanroepen' }
  ])

  const STEP_TYPES = Object.keys(STEP_TYPE_ICONS)

  function addStep() {
    setSteps(s => [...s, { type: 'api_call', label: 'Nieuwe stap' }])
  }
...
export default function WorkflowControls({ workflows }: { workflows: Workflow[] }) {
  ...
  return (
    <>
      ...
      <div className="space-y-2">
        {workflows.map(wf => (
          <WorkflowRow key={wf.id} wf={wf} defaultSteps={wf.steps as Array<{ type: string; label: string }>} />
        ))}
        {workflows.length === 0 && (
