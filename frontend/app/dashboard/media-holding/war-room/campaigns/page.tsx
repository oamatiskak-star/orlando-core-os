...
export default async function CampaignStudioPage() {
  ...
  // leads/sales/revenue per campagne (niche) uit affiliate_conversions via channel→niche
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-white/45">Campaign Studio — prestaties per campagne (niche) inclusief commerciële uitkomst. Affiliate-data nog 0 → Leads/Sales = &quot;Geen data&quot;.</p>
        ...
        {campaigns.map((c) => {
          ...
          return (
            <div key={c.campaign_key} className="rounded-lg border border-white/8 bg-[#0e1525] p-4">
              ...
              {/* Campaign tree: Campagne → Hooks → Creatives → Platforms */}
              ...
              <div className="mt-3 grid grid-cols-3 gap-2 text-center">
                ...
                <Stat icon={Video} label="retentie" value={c.avg_retention != null ? `${c.avg_retention}%` : '—'} />
              </div>
