...
export default async function WorkspaceLibraryPage() {
  ...
  const [nodesRes, edgesRes, hermesRes] = await Promise.all([
    ...
    supabase.from('executive_recommendations')
      .select('id, action_kind, target_id, priority, rationale, status')
      .eq('target_kind', 'channel').neq('status', 'executed')
      .order('priority', { ascending: false }).limit(600),
  ])
  ...
  if (nodesRes.error) {
    return <div className="rounded-lg border border-red-500/20 bg-red-500/5 p-4 text-sm text-red-300">Kon de library niet laden: {nodesRes.error.message}</div>
  }
  if (rawNodes.filter((n) => n.node_type === 'creative').length === 0) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-white/45">Creative Library — alle advertenties/creatives visueel, zoals een creative workspace. Klik een creative voor het detail-paneel.</p>
