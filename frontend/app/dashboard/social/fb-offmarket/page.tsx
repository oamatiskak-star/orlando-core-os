...
const PRIO_COLORS: Record<string, string> = {
  ...
  normaal:'text-blue-400 bg-blue-500/10 border-blue-500/20',
...
export default function FBOffMarketPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const sp = new URLSearchParams({ group_type: 'offmarket', limit: '100' })
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-white/65 mt-0.5">Off-market vastgoeddeals uit Facebook groep</p>
        </div>
        <button
          onClick={() => setModal({ ...EMPTY })}
          className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-blue-500/15 border border-blue-500/20 text-blue-400 hover:bg-blue-500/25 transition-colors text-sm"
        >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: 'Totaal',      value: total,                    icon: Home,         color: 'text-blue-400   bg-blue-500/10   border-blue-500/20' },
      ...
      {loading ? (
        ...
        <div className="bg-white/[0.04] border border-white/8 rounded-xl p-10 flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-white/40">Nog geen off-market deals geregistreerd</p>
          <button onClick={() => setModal({ ...EMPTY })} className="text-xs text-blue-400 hover:text-blue-300">Eerste deal toevoegen</button>
        </div>
      ) : (
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 mt-2 flex-wrap">
                    ...
                    {d.source_url && (
                      <a href={d.source_url} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-xs text-blue-400 hover:text-blue-300">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#111] border border-white/10 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto flex flex-col">
            <div className="sticky top-0 bg-[#111] border-b border-white/8 px-5 py-4 flex items-center justify-between">
              <h2 className="text-base font-semibold text-white">{modal.id ? 'Deal bewerken' : 'Deal toevoegen'}</h2>
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Details over de deal..."
                  className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder-white/25 focus:outline-none focus:border-blue-500/50 resize-none"
                />
              </div>
            ...
            <div className="sticky bottom-0 bg-[#111] border-t border-white/8 px-5 py-4 flex justify-end gap-3">
              ...
              <button
                ...
                className="px-4 py-2 rounded-lg text-sm bg-blue-500/20 border border-blue-500/30 text-blue-300 hover:bg-blue-500/30 transition-colors disabled:opacity-50"
              >
