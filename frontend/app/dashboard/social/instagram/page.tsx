...
import { Image, BarChart3, TrendingUp, Plus, Pencil, Trash2, Calendar, Hash, CheckCircle, Clock, XCircle, FileText } from 'lucide-react'
...
const TYPE_LABELS: Record<string, string> = {
  post: 'Post', reel: 'Reel', story: 'Story', short: 'Short',
}
...
export default function InstagramPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const sp = new URLSearchParams({ platform: 'instagram', limit: '100' })
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-white/65 mt-0.5">Content planner en publicatieschema</p>
        </div>
        <button
          onClick={() => setModal({ ...EMPTY })}
          className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-pink-500/15 border border-pink-500/20 text-pink-400 hover:bg-pink-500/25 transition-colors text-sm"
        >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: 'Totaal',     value: total,            icon: Image,       color: 'text-pink-400   bg-pink-500/10   border-pink-500/20' },
      ...
      {loading ? (
        ...
        <div className="bg-white/[0.04] border border-white/8 rounded-xl p-10 flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-white/40">Nog geen Instagram posts</p>
          <button onClick={() => setModal({ ...EMPTY })} className="text-xs text-pink-400 hover:text-pink-300">Eerste post aanmaken</button>
        </div>
      ) : (
            ...
            <div key={p.id} className="group bg-white/[0.06] border border-white/5 rounded-xl p-4 space-y-3 hover:border-white/15 transition-colors">
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="px-2 py-0.5 rounded-full text-[11px] bg-pink-500/10 border border-pink-500/20 text-pink-400">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#111] border border-white/10 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto flex flex-col">
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                <label className="text-xs text-white/50 mb-1 block">Caption</label>
                <textarea
                  rows={4}
                  value={modal.caption ?? ''}
                  onChange={e => setModal(m => ({ ...m, caption: e.target.value }))}
                  placeholder="Post caption..."
                  className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder-white/25 focus:outline-none focus:border-pink-500/50 resize-none"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="#vastgoed #investeren #nederland"
                  className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder-white/25 focus:outline-none focus:border-pink-500/50"
                />
              </div>
            ...
            <div className="sticky bottom-0 bg-[#111] border-t border-white/8 px-5 py-4 flex justify-end gap-3">
              ...
              <button
                onClick={save}
                disabled={saving}
                className="px-4 py-2 rounded-lg text-sm bg-pink-500/20 border border-pink-500/30 text-pink-300 hover:bg-pink-500/30 transition-colors disabled:opacity-50"
              >
