  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-white/40 mt-0.5">HR, contracten, loonstroken en medewerkersbeheer</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {MODULES.map(({ href, icon: Icon, label, desc }) => (
          <Link key={href} href={href}
            className="bg-white/[0.03] border border-white/8 rounded-xl p-5 space-y-2 hover:border-white/15 hover:bg-white/[0.05] transition-colors">
