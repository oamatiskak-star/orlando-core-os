...
// Vercel cron — /api/acquisition/cron/building-inspection
// Schedule: 0 2 * * * (dagelijks om 02:00)
// Beveiligd via Bearer CRON_SECRET.
//
// Triggert BuildingInspectionScraper worker op Render acquisition-engine
// Fetcht bouwveiligheid gegevens van BKWI/DVGO
// Analyseert inspecties, schendingen, veiligheidscertificaten
...
export async function GET(req: NextRequest) {
  ...
  try {
    const response = await fetch(`${engineUrl}/workers/building-inspection/run`, {
      method: 'POST',
    ...
    try {
      await admin
        .from('acq_scan_jobs')
        .insert({
          agent_name: 'BuildingInspectionScraper',
          job_type: 'building_inspection_enrich',
