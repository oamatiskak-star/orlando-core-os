...
// Vercel cron — /api/acquisition/cron/property-valuation
// Schedule: 0 */6 * * * (elke 6 uur)
// Beveiligd via Bearer CRON_SECRET.
//
// Triggert PropertyValuationScraper worker op Render acquisition-engine
// Verrijkt deals met waarderinganalyses: WOZ, historische prijzen, comparables, ROI
...
export async function GET(req: NextRequest) {
  ...
  try {
    const response = await fetch(`${engineUrl}/workers/property-valuation/run`, {
      method: 'POST',
    ...
    try {
      await admin
        .from('acq_scan_jobs')
        .insert({
          agent_name: 'PropertyValuationScraper',
          job_type: 'property_valuation_analysis',
