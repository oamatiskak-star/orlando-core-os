...
// Vercel cron — /api/acquisition/cron/funda-scan
// Schedule: 0 */4 * * * (elke 4 uur)
// Beveiligd via Bearer CRON_SECRET.
//
// Triggert FundaScraper worker op Render acquisition-engine
// Scraapt actuele woningaanbod van Funda API en voegt toe aan acq_deals.
...
export async function GET(req: NextRequest) {
  ...
  try {
    const response = await fetch(`${engineUrl}/workers/funda-scraper/run`, {
      method: 'POST',
