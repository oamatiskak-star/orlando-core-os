...
// Vercel cron — /api/acquisition/cron/predictive-models
// Schedule: 0 */4 * * * (elke 4 uur)
// Beveiligd via Bearer CRON_SECRET.
//
// Triggert PredictiveModelsScraperWorker op Render acquisition-engine
// Genereert ML-gebaseerde ROI-, prijs- en marktprognoses voor deals
...
export async function GET(req: NextRequest) {
  ...
  try {
    const response = await fetch(`${engineUrl}/workers/predictive-models/run`, {
      method: 'POST',
    ...
    try {
      await admin
        .from('acq_scan_jobs')
        .insert({
          agent_name: 'PredictiveModelsScraperWorker',
          job_type: 'predictive_models_analysis',
