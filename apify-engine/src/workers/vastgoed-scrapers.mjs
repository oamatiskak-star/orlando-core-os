...
const SCRAPER_CONFIGS = [
  ...
  {
    key: 'booking_amsterdam',
    actorKey: 'BOOKING_COM',
    country: 'NL',
    input: { location: 'Amsterdam, Netherlands', maxItems: 100, currency: 'EUR', stayType: 'apartments' },
    mapItem: item => ({
      external_id: String(item.id || item.hotelId || ''),
      source_name: 'booking_com',
      country: 'NL',
      city: 'Amsterdam',
      address: item.address || item.name || null,
      price: item.price || item.pricePerNight || null,
      price_currency: 'EUR',
      area_m2: null,
      property_type: item.propertyType || 'rental',
      url: item.url || (item.id ? `https://www.booking.com/hotel/nl/${item.id}.html` : null),
      raw: item,
    }),
  },
  {
    ...
    actorKey: 'BOOKING_COM',
    country: 'AE',
    input: { location: 'Dubai, UAE', maxItems: 100, currency: 'USD', stayType: 'apartments' },
    mapItem: item => ({
      external_id: String(item.id || item.hotelId || '') + '_bkdxb',
      source_name: 'booking_com',
      country: 'AE',
      city: 'Dubai',
      address: item.address || item.name || null,
  ...
  },
  {
    ...
    actorKey: 'BOOKING_COM',
    country: 'US',
    input: { location: 'New York, NY', maxItems: 100, currency: 'USD', stayType: 'apartments' },
    mapItem: item => ({
      external_id: String(item.id || item.hotelId || '') + '_bknyc',
      source_name: 'booking_com',
      country: 'US',
      city: 'New York',
      address: item.address || item.name || null,
  ...
  {
    key: 'vrbo_usa',
    actorKey: 'VRBO_SEARCH',
    country: 'US',
    input: { location: 'Miami, FL', maxResults: 100 },
    mapItem: item => ({
      external_id: String(item.id || item.propertyId || item.listingId || ''),
      source_name: 'vrbo',
      country: 'US',
      city: item.city || 'Miami',
      address: item.name || item.headline || item.address || null,
      price: item.price || item.nightlyPrice || null,
      price_currency: 'USD',
      area_m2: item.sqft ? Math.round(item.sqft * 0.0929) : null,
      property_type: item.propertyType || item.type || 'rental',
      url: item.url || (item.id ? `https://www.vrbo.com/${item.id}` : null),
      raw: item,
    }),
  },
  {
    key: 'uk_property',
    actorKey: 'UK_PROPERTY',
    country: 'GB',
    input: { location: 'London', maxItems: 100, listingType: 'sale' },
    mapItem: item => ({
      external_id: String(item.id || item.listingId || item.propertyId || ''),
      source_name: 'uk_property',
      country: 'GB',
      city: item.city || item.location?.town || 'London',
      address: item.address || item.displayAddress || item.title || null,
      price: item.price?.amount || item.price || null,
      price_currency: 'GBP',
      area_m2: item.floorAreaSqFt ? Math.round(item.floorAreaSqFt * 0.0929) : null,
      property_type: item.propertyType || item.type || 'house',
      url: item.url || null,
      raw: item,
    }),
  },
  {
    key: 'furnished_finder_usa',
    actorKey: 'FURNISHED_FINDER',
    country: 'US',
    input: { location: 'Austin, TX', maxItems: 100 },
    mapItem: item => ({
      external_id: String(item.id || item.listingId || ''),
      source_name: 'furnished_finder',
      country: 'US',
      city: item.city || 'Austin',
      address: item.address || item.title || null,
      price: item.price || item.monthlyRent || null,
      price_currency: 'USD',
      area_m2: item.sqft ? Math.round(item.sqft * 0.0929) : null,
      property_type: 'furnished_rental',
      url: item.url || null,
      raw: item,
    }),
  },
  {
    ...
    actorKey: 'AGODA',
    country: 'SG',
    input: { location: 'Singapore', maxItems: 100, propertyType: 'apartments' },
    mapItem: item => ({
      external_id: String(item.id || item.hotelId || item.propertyId || ''),
      source_name: 'agoda',
      country: 'SG',
      city: item.city || 'Singapore',
      address: item.name || item.address || null,
      price: item.price || item.pricePerNight || item.lowestPrice || null,
      price_currency: 'SGD',
      area_m2: null,
      property_type: item.propertyType || 'rental',
      url: item.url || (item.hotelId ? `https://www.agoda.com/property/${item.hotelId}.html` : null),
      raw: item,
    }),
  },
]
