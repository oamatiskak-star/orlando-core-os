...
exports.runOffMarketAI = runOffMarketAI;
...
async function runOffMarketAI() {
    ...
    for (const lead of leads ?? []) {
        try {
            ...
            logger_1.logger.error(`OffMarketAI lead ${lead.id} failed`, { err: String(err) });
        }
    }
    // Update tasks_done
    if (processed > 0) {
        try {
            const { error } = await supabase_1.supabase.rpc('acq_agent_increment_done', { agent_name: AGENT, n: processed });
            if (error)
                throw error;
        }
        catch (err) {
            ...
            try {
                await supabase_1.supabase.from('acq_agent_registry')
    ...
    logger_1.logger.info(`OffMarketAI run done`, { processed, duration_ms: Date.now() - start });
...
//# sourceMappingURL=offmarket-ai.js.map