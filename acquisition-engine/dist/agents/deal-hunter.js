...
exports.runDealHunter = runDealHunter;
...
async function runDealHunter() {
    ...
    const { data: jobs } = await supabase_1.supabase
        .from('acq_scan_jobs')
        .select('*')
        .eq('agent_name', AGENT)
        .eq('status', 'queued')
    ...
    for (const job of pending) {
        ...
        try {
            ...
            const { data: unscored } = await supabase_1.supabase
                .from('acq_deals')
            ...
            for (const deal of unscored ?? []) {
                ...
                await supabase_1.supabase
                    ...
                    .eq('id', deal.id);
                await supabase_1.supabase.from('acq_deal_scores').insert({
                    deal_id: deal.id,
                    score,
            ...
            logger_1.logger.error(`DealHunter job ${job.id} failed`, { err: String(err) });
        }
    }
    await setAgentStatus(AGENT, 'idle');
    logger_1.logger.info(`DealHunter run done`, { processed, duration_ms: Date.now() - start });
...
function computeBasicScore(deal) {
    ...
    if (deal.roi_pct !== null) {
        if (deal.roi_pct >= 15)
            score += 25;
        else if (deal.roi_pct >= 10)
            score += 18;
        else if (deal.roi_pct >= 6)
            score += 10;
        else
            score -= 5;
    }
    if (deal.area_m2 !== null) {
        if (deal.area_m2 >= 200)
            score += 10;
        else if (deal.area_m2 >= 100)
            score += 5;
    }
    const goodTypes = ['loods', 'kantoor', 'woning', 'appartement'];
    if (deal.object_type && goodTypes.includes(deal.object_type))
        score += 8;
    const badLabels = ['F', 'G'];
    if (deal.energy_label && badLabels.includes(deal.energy_label))
        score -= 8;
    if (deal.build_year && deal.build_year < 1970)
        score -= 5;
    return Math.max(0, Math.min(100, score));
}
function computeRiskScore(deal) {
    let risk = 30;
    if (deal.asking_price !== null && deal.asking_price > 1000000)
        risk += 15;
    if (deal.energy_label && ['F', 'G'].includes(deal.energy_label))
        risk += 20;
    if (deal.build_year && deal.build_year < 1950)
        risk += 15;
...
async function completeJob(id, resultCount) {
    await supabase_1.supabase
        .from('acq_scan_jobs')
    ...
    await supabase_1.supabase
        .from('acq_agent_registry')
        .update({ tasks_done: supabase_1.supabase.rpc('increment', { x: 1 }) })
        .eq('name', 'DealHunter');
}
async function failJob(id, error) {
    ...
    await supabase_1.supabase
        .from('acq_agent_registry')
        .update({ tasks_failed: supabase_1.supabase.rpc('increment', { x: 1 }) })
        .eq('name', 'DealHunter');
}
//# sourceMappingURL=deal-hunter.js.map