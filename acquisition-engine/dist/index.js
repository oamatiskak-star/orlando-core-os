...
const environmental_risk_scraper_1 = require("./workers/environmental-risk-scraper");
const neighborhood_analytics_scraper_1 = require("./workers/neighborhood-analytics-scraper");
...
app.post('/workers/immobelt-scraper/run', async (_req, res) => {
    try {
        const result = await withAgentGuard('ImmobeltScraper', immobelt_scraper_1.runImmobeltScraper);
        res.json({ status: 'ok', ...result });
    }
    catch (err) {
        res.status(500).json({ status: 'error', error: err.message });
...
app.post('/workers/building-inspection/run', async (_req, res) => {
    try {
        const result = await withAgentGuard('BuildingInspectionScraper', building_inspection_scraper_1.runBuildingInspectionScraper);
        res.json({ status: 'ok', ...result });
    }
    catch (err) {
        res.status(500).json({ status: 'error', error: err.message });
...
app.post('/workers/environmental-risk/run', async (_req, res) => {
    try {
        const result = await withAgentGuard('EnvironmentalRiskScraper', environmental_risk_scraper_1.runEnvironmentalRiskScraper);
        res.json({ status: 'ok', ...result });
    }
    catch (err) {
        res.status(500).json({ status: 'error', error: err.message });
...
app.post('/workers/neighborhood-analytics/run', async (_req, res) => {
    try {
        const result = await withAgentGuard('NeighborhoodAnalyticsScraper', neighborhood_analytics_scraper_1.runNeighborhoodAnalyticsScraper);
        res.json(result);
    }
    catch (err) {
        res.status(500).json({ status: 'error', error: err.message });
...
node_cron_1.default.schedule('0 */8 * * *', () => {
    withAgentGuard('EnvironmentalRiskScraper', environmental_risk_scraper_1.runEnvironmentalRiskScraper)
        .catch(err => logger_1.logger.error('Scheduled EnvironmentalRiskScraper failed', { err: String(err) }));
}, { timezone: TZ });
// NeighborhoodAnalyticsScraper: elke 6 uur buurt-analyses (scholen, criminaliteit, openbaar vervoer, demografie)
node_cron_1.default.schedule('0 */6 * * *', () => {
    withAgentGuard('NeighborhoodAnalyticsScraper', neighborhood_analytics_scraper_1.runNeighborhoodAnalyticsScraper)
        .catch(err => logger_1.logger.error('Scheduled NeighborhoodAnalyticsScraper failed', { err: String(err) }));
}, { timezone: TZ });
// ── Start ─────────────────────────────────────────────────────────────────────
app.listen(PORT, () => {
    logger_1.logger.info(`Acquisition Engine started on :${PORT} (tz=${TZ})`);
    logger_1.logger.info('18 cron schedules: DealHunter, OffMarketAI, PermitAI, MunicipalityAI, InvestorAI, OutreachAI, RiskAI, AcquisitionDirectorAI, FundaScraper, KadasterScraper, PermitsScraper, ImmobeltScraper, KvKCompanyProfiler, SpatialPlanningScraper, BuildingInspectionScraper, MarketAnalysisScraper, EnvironmentalRiskScraper, NeighborhoodAnalyticsScraper');
});
process.on('SIGTERM', () => {
