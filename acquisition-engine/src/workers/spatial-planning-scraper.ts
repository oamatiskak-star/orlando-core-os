...
/**
 * Spatial Planning Data Scraper — RUD (Ruimtelijke Uitvoeringsdata)
 * Fetches Dutch municipal spatial planning information
 * from ruimtelijkeplannen.nl (STTR Thesaurus registry)
 *
 * Rate limit: 100 req/hour per municipality
 * No authentication required — public WFS/WMS services
 */

interface SpatialPlanningZone {
  id: string
  municipality: string
  zone_name: string
  zone_type: string // 'residential', 'commercial', 'mixed-use', 'industrial', 'agricultural'
  designation: string // Dutch: 'woongebied', 'bedrijventerrein', etc.
  allowed_uses: string[]
  development_potential: 'high' | 'medium' | 'low' | 'restricted'
  height_restriction?: number
  building_ratio?: number
  green_space_ratio?: number
  parking_spaces_required?: number
  status: string // 'approved', 'draft', 'expired'
  valid_from?: string
  valid_until?: string
  geometry?: {
    type: string
...
export class SpatialPlanningScraper extends ScraperBase {
  private httpClient: HttpClient

  // RUD API endpoints — WFS (Web Feature Service) for spatial data
  private readonly RUD_WFS_URL = 'https://www.ruimtelijkeplannen.nl/geonetwork/wfs'
  private readonly RUD_SEARCH_URL = 'https://www.ruimtelijkeplannen.nl/web/guest/plannen/list'
  ...
  async run(): Promise<ScraperResult> {
    ...
    try {
      logger.info('SpatialPlanningScraper starting')
      ...
      logger.info(`SpatialPlanningScraper found ${totalFound} municipalities needing enrichment`)

      // Process each municipality
      const enrichments: SpatialEnrichment[] = []
      for (let i = 0; i < municipalities.length; i++) {
        try {
          ...
          logger.warn(`Error processing municipality ${municipalities[i]}`, {
            error: String(err),
          })
        }
      }
      ...
      logger.info('SpatialPlanningScraper completed', {
        found: totalFound,
      ...
      logger.error('SpatialPlanningScraper failed', { error, duration_ms: Date.now() - startTime })
   ...
   */
  private async fetchUniqueMunicipalities(): Promise<string[]> {
    try {
      ...
      logger.error('Failed to fetch municipalities', { error: String(err) })
      return []
    }
  }
   ...
   */
  private async fetchSpatialPlanData(municipality: string): Promise<SpatialPlanningZone[]> {
    const zones: SpatialPlanningZone[] = []
    ...
    try {
      ...
      const response = await this.retryWithBackoff(
        () =>
          ...
          }>(`${this.RUD_WFS_URL}?${queryString}`),
        `Fetch RUD zones for ${municipality}`
      )
      ...
      for (const feature of response.features) {
        try {
          ...
          logger.warn(`Error mapping RUD feature for ${municipality}`, { error: String(err) })
        }
      }

      return zones
    } catch (err) {
      logger.error(`Failed to fetch RUD data for ${municipality}`, { error: String(err) })
      return zones
    }
  }
   ...
   */
  private mapFeatureToZone(feature: any, municipality: string): SpatialPlanningZone | null {
   ...
   */
  private normalizeZoneType(dutchType: string): string {
   ...
   */
  private extractAllowedUses(zoneType: string): string[] {
  ...
  private assessDevelopmentPotential(
    ...
    const potentialMap: Record<string, 'high' | 'medium' | 'low' | 'restricted'> = {
   ...
   */
  private async fetchDealsInMunicipality(municipality: string): Promise<Array<{ id: string }>> {
    try {
      const { data, error } = await supabase
        .from('acq_deals')
        .select('id')
      ...
      logger.error(`Failed to fetch deals in ${municipality}`, { error: String(err) })
      return []
    }
  }
   ...
   */
  private async insertSpatialEnrichments(
    enrichments: SpatialEnrichment[]
    ...
    try {
      ...
      const { error } = await supabase.from('acq_spatial_planning').upsert(enrichments, {
        onConflict: 'id',
      })
      ...
      logger.error('Failed to insert spatial enrichments', { error: String(err) })
 ...
 */
export async function runSpatialPlanningScraper(): Promise<{
  agent: string
  itemsFound: number
