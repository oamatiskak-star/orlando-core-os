 ...
 * Uses app-facing APIs (pyfunda reverse-engineered endpoints)
 * No authentication required - operates anonymously like the mobile app
 *
 * Implements aggressive rate limiting (500ms-1s delays) to avoid IP blocking
 */

interface FundaSearchParams {
  city?: string
  provinces?: string[]
  priceMin?: number
  priceMax?: number
  page?: number
  pageSize?: number
  propertyTypes?: string[] // 'Appartement', 'Eengezinswoning', etc.
}
...
export class FundaScraperWorker extends ScraperBase {
  ...
  private readonly SEARCH_ENDPOINT = 'https://listing-search-wonen.funda.io/_msearch/template'
  private readonly DETAIL_ENDPOINT = 'https://listing-detail-page.funda.io/api/v4/listing/object/nl'
  ...
  async run(): Promise<ScraperResult> {
    ...
    try {
      ...
      const searchParams: FundaSearchParams = {
        pageSize: 50,
      }

      logger.info('FundaScraper starting', { params: searchParams })

      // Fetch listings (paginate through results)
      const listings = await this.fetchAllListings(searchParams)
      totalFound = listings.length

      logger.info(`FundaScraper fetched ${totalFound} listings`, { duration_ms: Date.now() - startTime })
      ...
      logger.info('FundaScraper completed', {
        found: totalFound,
      ...
      logger.error('FundaScraper failed', { error, duration_ms: Date.now() - startTime })
   ...
   */
  private async fetchAllListings(params: FundaSearchParams): Promise<FundaListing[]> {
    const allListings: FundaListing[] = []
    let page = 1
    ...
    while (hasMore && page <= maxPages) {
      try {
        ...
        const response = await this.retryWithBackoff(
          () => this.searchFundaListings(page, params),
          `Fetch Funda page ${page}`
        )
        ...
        const listings = response as FundaListing[]
        ...
        logger.warn(`Error on Funda page ${page}`, { error: String(err) })
        hasMore = false
      }
    }
   ...
   */
  private async searchFundaListings(
    page: number,
    params: FundaSearchParams
  ): Promise<FundaListing[] | null> {
    try {
      ...
      const response = await this.httpClient.post<{
        responses: Array<{
          hits: {
            hits: Array<{
              _source: FundaListing
            }>
          }
        }>
      }>(this.SEARCH_ENDPOINT, query)
      ...
      logger.error('Failed to search Funda listings', { error: String(err) })
      return null
    }
  }
   ...
   */
  private buildSearchQuery(page: number, params: FundaSearchParams): Record<string, unknown> {
   ...
   */
  private mapListingToDeal(listing: FundaListing): RawDeal {
    return {
   ...
   */
  private extractProvinceFromPostalCode(postalCode?: string): string | undefined {
 ...
 */
export async function runFundaScraper(): Promise<{ agent: string; itemsFound: number; itemsInserted: number }> {
