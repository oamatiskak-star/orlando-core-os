...
export async function runBuildOppsScanner(): Promise<AgentRunResult> {
  ...
  for (const opp of opps ?? []) {
    try {
      ...
      if (newStage === 'analyse') promoted++
      processed++
    } catch (err) {
      logger.error(`BuildOppsScanner opp ${opp.id} failed`, { err: String(err) })
    }
  }

  await setAgentStatus(AGENT, 'idle')
  logger.info(`BuildOppsScanner done`, { processed, promoted, duration_ms: Date.now() - start })
...
}): Promise<{ relevance_score: number; strategy: string }> {

  // Snelle rule-based score eerst
  let score = 30
  if (opp.estimated_value) {
    if (opp.estimated_value > 5_000_000)  score += 30
    else if (opp.estimated_value > 1_000_000) score += 20
    else if (opp.estimated_value > 250_000)   score += 10
  }
  const goodTypes = ['renovatie', 'nieuwbouw', 'transformatie', 'uitbouw']
  if (opp.opp_type && goodTypes.includes(opp.opp_type)) score += 15
  if (opp.deadline) {
    const daysLeft = (new Date(opp.deadline).getTime() - Date.now()) / 86400000
    if (daysLeft > 14 && daysLeft < 90) score += 10
  }
  const goodProvinces = ['Noord-Holland', 'Zuid-Holland', 'Utrecht', 'Noord-Brabant', 'Gelderland']
  if (opp.province && goodProvinces.includes(opp.province)) score += 10
