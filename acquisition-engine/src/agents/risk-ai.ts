...
export async function runRiskAI(): Promise<AgentRunResult> {
  ...
  logger.info(`RiskAI run done`, { processed, duration_ms: Date.now() - start })
...
function computeDetailedRisk(deal: {
  ...
  if (deal.asking_price && deal.estimated_value) {
    ...
    if (ratio > 1.1)  risk += 20  // overprijsd
    if (ratio < 0.85) risk += 10  // te goedkoop = verborgen gebreken risico
  }
  if (deal.asking_price && deal.asking_price > 2_000_000) risk += 10  // groot ticket

  // Technisch risico
  if (deal.energy_label && ['F', 'G'].includes(deal.energy_label)) risk += 15
  if (deal.build_year && deal.build_year < 1940) risk += 15
  else if (deal.build_year && deal.build_year < 1970) risk += 8

  // Marktrisico
  const highRiskTypes = ['horeca', 'winkel', 'industrie']
  if (deal.object_type && highRiskTypes.includes(deal.object_type)) risk += 12

  // Pipeline risico (hoe later hoe meer sunk cost risk)
  if (deal.pipeline_stage === 'due_diligence') risk += 5
  if (deal.pipeline_stage === 'bod') risk += 8
