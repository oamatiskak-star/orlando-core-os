...
exports.checkLocalFleet = checkLocalFleet;
...
async function checkLocalFleet(opts) {
    let apps;
    try {
        apps = await (0, pm2_1.listPM2Apps)();
    }
    catch (err) {
    ...
    for (const app of apps) {
        if (app.name === opts.selfAppName)
            continue;
        if (opts.denyList.has(app.name))
            continue;
        checked++;
        try {
            const failed = await checkApp(app, opts);
            if (failed)
                failing.push(app.name);
        }
        catch (err) {
...
async function checkApp(app, opts) {
    ...
    if (status === 'online') {
        if (state.lastStatus && state.lastStatus !== 'online' && recoveryActive) {
            await (0, telegram_1.sendTelegram)('info', `✅ ${app.name} recovered`, `Back online on ${opts.hostId} after ${state.rebuildAttempts} rebuild(s).`);
            await (0, supabase_state_1.recordEvent)({
                host_id: opts.hostId,
                service_id: `pm2:${app.name}`,
                service_name: app.name,
            ...
            if (state.incidentKey)
                await (0, supabase_state_1.resolveIncident)(opts.hostId, state.incidentKey);
    ...
    if (state.restartTimestamps.length >= CRASH_LOOP_THRESHOLD) {
        ...
        if (!state.escalated)
            await escalate(app, state, opts, 'crash-loop persists after rebuilds');
        return true;
    }
    // Simple restart path with cooldown
    if (Date.now() - state.lastWatchdogRestartAt < RESTART_COOLDOWN_MS)
        return true;
    state.lastWatchdogRestartAt = Date.now();
    await fireEvent(app, opts, 'fail_detected', `status=${status} restart_count=${restartCount}`);
    await (0, telegram_1.sendTelegram)('error', `🔴 ${app.name} on ${opts.hostId} is ${status}`, `Restart count=${restartCount}. Triggering pm2 restart.`);
    try {
        await (0, pm2_1.restartApp)(app.name);
...
async function handleCrashLoop(app, state, opts) {
    if (Date.now() - state.lastRebuildAt < REBUILD_COOLDOWN_MS)
        return;
    state.lastRebuildAt = Date.now();
    state.rebuildAttempts += 1;
    const attempt = state.rebuildAttempts;
    await (0, telegram_1.sendTelegram)('warning', `🔁 ${app.name} crash-loop on ${opts.hostId}`, `${state.restartTimestamps.length} restarts in ${Math.round(CRASH_LOOP_WINDOW_MS / 60000)} min. Rebuild attempt ${attempt}/${MAX_REBUILDS}.`);
    let logTail = '';
    try {
        logTail = await (0, pm2_1.tailErrorLog)(app, 80);
    }
    catch {
        /* ignore */
    }
    ...
    try {
        await (0, pm2_1.stopApp)(app.name);
    }
    catch (err) {
        console.error('[local-watchdog] stop failed:', err);
    }
    const rb = await (0, pm2_1.rebuildApp)(app);
    if (rb.error) {
        await (0, telegram_1.sendTelegram)('error', `🔴 ${app.name} rebuild failed`, rb.log.slice(-1500));
    }
    try {
        await (0, pm2_1.startApp)(app.name);
    }
    catch (err) {
        await (0, telegram_1.sendTelegram)('error', `🔴 ${app.name} start failed after rebuild`, err instanceof Error ? err.message : String(err));
    }
...
async function escalate(app, state, opts, reason, logTail = '') {
    state.escalated = true;
    const incidentKey = `${opts.hostId}:pm2:${app.name}:${Date.now()}`;
    state.incidentKey = incidentKey;
    const tail = logTail || (await (0, pm2_1.tailErrorLog)(app, 80).catch(() => ''));
    const proposed = [
    ...
    await (0, telegram_1.sendTelegram)('critical', `🚨 ${app.name} needs human/Claude Code fix on ${opts.hostId}`, [
...
async function fireEvent(app, opts, kind, message, logsTail) {
    await (0, supabase_state_1.recordEvent)({
        host_id: opts.hostId,
        service_id: `pm2:${app.name}`,
        service_name: app.name,
